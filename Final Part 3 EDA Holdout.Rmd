---
title: 'Final Part 1 EDA Holdout'
...
df <- read.csv("../data/holdout.csv", stringsAsFactors = TRUE)
head(df)
```
...
```{r}
# collapse `application_type` into "INDIVIDUAL" and "OTHER"
# df$application_type_collapsed <- forcats::fct_other(df$application_type, keep = "INDIVIDUAL", other_level = "OTHER")
...
# df <- df %>% dplyr::select(-application_type)
```

**We chose to collapse `application_type` into "INDIVIDUAL" and "OTHER" as the other levels are extremely sparse. We chose to remove `application_type` due to dominance of "INDIVIDUAL".**

```{r}
# collapse `purpose` into the top 3 levels in terms of frequency and "OTHER" ("OTHER" includes all less frequent levels)
df$purpose <- forcats::fct_collapse(df$purpose, OTHER = c("renewable_energy", "educational", "wedding", "house", "other", "car", "moving", "vacation", "small_business", "medical"))
...
**We chose to collapse `purpose` into the top 3 levels in terms of frequency and "OTHER" to mitigate data sparsity and draw predictive power. We also chose to relevel `purpose` so that the highest frequency level ("debt_consolidation") is set as the base level.**
...
df <- df %>%
  mutate(
    emp_length = str_trim(as.character(emp_length)),
    emp_length_grouped = fct_collapse(
      emp_length,
      "Unreported" = "",
      "<2yr"   = c("< 1 year", "1 year"),
      "2-3yr"  = c("2 years", "3 years"),
      "4-5yr"  = c("4 years", "5 years"),
      "6-7yr"  = c("6 years", "7 years"),
      "8-9yr"  = c("8 years", "9 years"),
      "10+yr"  = "10+ years"
    )
  )
...
df$emp_length_grouped <- fct_relevel(df$emp_length_grouped, "<2yr")
...
df$home_ownership_collapsed <- forcats::fct_collapse(df$home_ownership, OTHER = c("OTHER", "NONE", "ANY"))

# drop "OTHER" level
df <- droplevels(df[!df$home_ownership_collapsed == "OTHER",])

table(df$home_ownership_collapsed)
```
...
df$issue_d_years_since_grouped <- df$issue_d_years_since %>%
  as.character() %>%
  fct_collapse(
    "9" = "9",
    "10" = "10",
    ...
    "13" = "13",
    "14" = "14",
    "15" = as.character(15:max(df$issue_d_years_since))) # range from 10 to 18

# remove `issue_d` and `issue_d_date`
df <- df %>% dplyr::select(-issue_d, -issue_d_date, -issue_d_years_since)

df$issue_d_years_since_grouped <- fct_relevel(df$issue_d_years_since_grouped, "9")

table(df$issue_d_years_since_grouped)
```
...
#   df$open_acc,
#   breaks = c(0, 6, 7, 8, 9, 10, 12, 13, 15, 18, 90), # training set breakpoints
#   include.lowest = TRUE,
#   right = TRUE
# )

# df <- df %>% select(-open_acc)

# table(df$open_acc_bins)
# table(df$open_acc)

# hist(df$open_acc_log)
```
...
df$earliest_cr_line_years_since <- pmin(df$earliest_cr_line_years_since, 75)

breaks_train <- c(12,19,21,23,24,26,27,30,32,37,75)
...
# define a look up table for all abbreviations
state_lookup <- data.frame(
  state = c(state.abb, "DC", "AA", "AE", "AP"),
  region = c(as.character(state.region), "South", "Other", "Other", "Other")
)

# add region
df <- df %>%
  left_join(state_lookup, by = c("territory" = "state"))

# # remove `territory`
# df <- df %>% dplyr::select(-territory)
...
```{r}
# recode `delinq_2yrs` into 2 levels: "0", "1+"
df$delinq_2yrs_collapsed <- forcats::fct_collapse(
  df$delinq_2yrs %>% as.character(), 
  "0" = "0",
  "1" = "1",
  "2+" = as.character(2:max(df$delinq_2yrs))) # range from 1 to the max value of the variable

# reorder levels of `delinq_2yrs`
df$delinq_2yrs_collapsed <- forcats::fct_relevel(df$delinq_2yrs_collapsed, "0", "1", "2+")

df <- df %>% select(-delinq_2yrs)

table(df$delinq_2yrs_collapsed)
```

**`delinq_2yrs` is zero-dominated and thus right-skewed. To combat this, we chose to convert `delinq_2yrs` to factor and collapse the categories into 2 levels: "0", "1+". We did this to reduce the number of levels, mitigate data sparsity, and classify large values of `delinq_2yrs` as equal.**

```{r}
# recode `inq_last_6mths` into 3 levels: "0", "1", "2+"
# df$inq_last_6mths_collapsed <- forcats::fct_collapse(
#   df$inq_last_6mths %>% as.character(), 
#   "0" = "0",
#   "1" = "1",
#   "2" = "2",
#   "3+" = as.character(3:max(df$inq_last_6mths))) # range from 2 to the max value of the variable

# reorder the levels of `inq_last_6mths`
# df$inq_last_6mths_collapsed <- forcats::fct_relevel(df$inq_last_6mths_collapsed, "0", "1", "2", "3+")

df$inq_last_6mths_log <- log1p(df$inq_last_6mths)

df <- df %>% select(-inq_last_6mths)

# table(df$inq_last_6mths)
```

**`inq_last_6mths` is zero-dominated and right-skewed. To combat this, we chose to convert `inq_last_6mths` to factor and collapse the categories into 3 levels: "0", "1", and "1+". We did this to reduce the number of levels, mitigate data sparsity, and classify larger values of `inq_last_6mths` as equal.**

```{r}
# recode `acc_now_delinq` into 2 levels: "0", "1+"
df$acc_now_delinq_collapsed <- forcats::fct_collapse(
...
df$acc_now_delinq_collapsed <- forcats::fct_relevel(df$acc_now_delinq_collapsed, "0", "1+")

# table(df$acc_now_delinq)

# remove `acc_now_delinq`
df <- df %>% dplyr::select(-acc_now_delinq)
```

**`acc_now_delinq` is extremely zero-dominated and right-skewed. To combat this, we chose to convert `acc_now_delinq` to factor and collapse the categories into 2 levels: "0", "1+". We did this to reduce the number of levels, mitigate data sparsity, and classify larger values of `acc_now_delinq` as equal. We chose to remove `acc_now_delinq` because of "0" dominance.**
...
**`annual_inc` is severely right-skewed with many large outliers. We chose to perform a log transformation on `annual_inc` to make the distribution close to normal and more symmetric. Depending on the performance of `annual_inc_log` in the modeling phase, we may choose to backtrack and cap `annual_inc` to the 99.5th percentile instead to limit the influence of the extreme outliers on future analysis. For tree-based models, we chose to reverse the log transformation and keep `annual_inc` as the raw, continuous predictor.**
...
df$pub_rec_non_bankruptcies_collapsed <- forcats::fct_collapse(
...
df$closed_acc_bins <- cut(
  df$closed_acc,
  breaks = c(0, 4, 6, 8, 10, 13, 15, 18, 21, 26, 110), # training set breakpoints
  include.lowest = TRUE,
  right = TRUE
)
...
df <- df %>% dplyr::select(-total_acc, -closed_acc, -open_acc)
# table(df$closed_acc)

# summary(df$closed_acc)
```
...
# drop rows with blank, now missing, values of `last_credit_pull_d_years_since`
df <- df[!is.na(df$last_credit_pull_d_years_since), ]

# regroup `last_credit_pull_d_years_since` to year ranges
df$last_credit_pull_d_years_since_grouped <- df$last_credit_pull_d_years_since %>%
  as.character() %>%
  fct_collapse(
    "6" = "6",
    "7" = "7",
...
df$last_credit_pull_d_years_since_grouped <- forcats::fct_relevel(df$last_credit_pull_d_years_since_grouped, "6", "7", "8", "9", "10+")

# remove `last_credit_pull_d` and `last_credit_pull_d_date`
df <- df %>% dplyr::select(-last_credit_pull_d, -last_credit_pull_d_date, -last_credit_pull_d_years_since)

table(df$last_credit_pull_d_years_since_grouped)
```

**In addition to the date values of `last_credit_pull_d`, there are 12 blank values. We chose to remove the rows with blank values of `last_credit_pull_d` as although they are an indicator of a loan applicant never having their credit pulled, there are too few observations to draw meaning and/or predictive power from this insight. `last_credit_pull_d` has 137 different combinations of months and years spanning from June 2007 to March 2019. To reduce this variable to something more interpretable, we chose to convert the issue date to a "years since" idea, reducing the total levels and assuming the month of last credit pull has little to no significance relative to year. Additionally, we chose to recode `last_credit_pull_d_years_since` into year ranges to again reduce the number of levels and mitigate data sparsity. For tree-based models, we chose to reverse the recoding and keep `last_credit_pull_d_years_since` as a continuous predictor.**
...
df$term <- as.factor(ifelse(df$term == " 36 months", "3_Years", "5_Years"))
...
df$mort_acc_NA <- factor(ifelse(is.na(df$mort_acc), 1, 0))
...
df$mort_acc_grouped <- dplyr::case_when(
  df$mort_acc == 0 ~ "0",
  df$mort_acc == 1 ~ "1",
  df$mort_acc == 2 ~ "2",
  df$mort_acc %in% 3:4 ~ "3-4",
  df$mort_acc >= 5 ~ "5+"
)

df$mort_acc_grouped <- factor(df$mort_acc_grouped, levels = c("0", "1", "2", "3-4", "5+"))
df <- df %>% select(-mort_acc, -home_ownership)
table(df$mort_acc_grouped)
```
...
* **We have also considered probable two-variable interactions within the data, however, we have not added any into the data frame yet. Depending on the types of predictive models we fit later on, we intend to either add in interactions (e.g. logistic regression) or account for implied interactions (e.g. random forest). Below are the top four most probable two-variable interactions in the data that we believe would have predictive power.**
...
df <- df %>%
  mutate(int_loan_amnt_revol_bal_log = loan_amnt * revol_bal_log)
```

* **In our EDA, we did not consider the predictive power or significance of variables by fitting any form of logistic regression model. We chose to rely on logic, domain knowledge, and visual insights in making our initial data cleaning, dimension reduction, and encoding decisions. Therefore, we acknowledge there may be a few variables still present in the data that will prove to be uninformative and lack predictive power upon model fitting.**
...
```{r}
# remove `territory` and `region`
df <- df %>% dplyr::select(-region, -territory)
```
...
```{r}
saveRDS(df, file = "../data/final_cleaned_holdout_part3.rds")
```
