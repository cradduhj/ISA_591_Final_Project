...
df$purpose <- forcats::fct_relevel(df$purpose, names(which.max(table(df$purpose))))
...
df$issue_d_years_since <- year(Sys.Date()) - year(df$issue_d_date)

# recategorizing `issue_d` into months of the year
# df$issue_d_months <- substr(df$issue_d, 0, 3)
...
# capping `open_acc`
# quantile(df$open_acc, probs = seq(0.9, 1, 0.001))
# cap_value <- quantile(df$open_acc, 0.998)
# df$open_acc_cap <- ifelse(df$open_acc > cap_value, cap_value, df$open_acc)

# combining `open_acc` into 10 bins
...
df$earliest_cr_line_years_since <- year(Sys.Date()) - year(df$earliest_cr_line_date)

# combining `earliest_cr_line_years_since` into 10 bins
...
# df$zip_code <- factor(substr(df$address, nchar(df$address) - 4, nchar(df$address)))

# table(df$zip_code)

# pull out `territory` from `address`
df$territory <- factor(substr(df$address, nchar(df$address) - 7, nchar(df$address) - 6))
...
```{r}
# recode `delinq_2yrs` into 4 levels: "0", "1", "2", "2+"
df$delinq_2yrs <- forcats::fct_collapse(
  df$delinq_2yrs %>% as.character(), 
  "0" = "0",
  "1" = "1",
  "2" = "2",
  "2+" = as.character(3:max(df$delinq_2yrs)))

# reorder levels of `delinq_2yrs`
df$delinq_2yrs <- forcats::fct_relevel(df$delinq_2yrs, "0", "1", "2", "2+")

table(df$delinq_2yrs)
```

**`delinq_2yrs` is zero-dominated and thus right-skewed. To combat this, we chose to convert `delinq_2yrs` to factor and collapse the categories into 4 levels: "0", "1", "2", and "2+". We did this to reduce the number of levels, mitigate data sparsity, and classify large values of `delinq_2yrs` as equal.**

```{r}
# recode `inq_last_6mths` into 3 levels: "0", "1", "1+"
df$inq_last_6mths <- forcats::fct_collapse(
  df$inq_last_6mths %>% as.character(), 
  "0" = "0",
  "1" = "1",
  "1+" = as.character(2:max(df$inq_last_6mths)))

# reorder the levels of `inq_last_6mths`
df$inq_last_6mths <- forcats::fct_relevel(df$inq_last_6mths, "0", "1", "1+")
...
df$acc_now_delinq <- forcats::fct_collapse(
  ...
  "0" = "0",
  "1+" = as.character(1:max(as.numeric(df$acc_now_delinq))))
...
df$sub_grade_num <- as.numeric(gsub("[A-G]", "", df$sub_grade))

# remove `sub_grade`
df <- df %>% dplyr::select(-sub_grade)
```

**`grade` has 7 levels with values of "A" through "G". `sub_grade` adds a number from 1 to 5 on top of these alphabetical values as a more granular risk rating within each grade. We chose to remove the alphabetical values from `sub_grade` as they are already included in `grade`, allowing `sub_grade` to be treated as continuous. Depending on the performance of `sub_grade_num` in the modeling phase, we may choose to backtrack and drop `grade` instead as the values of `grade` are included within `sub_grade`.**

```{r}
# log transforming `annual_inc`
df$annual_inc_log <- log(df$annual_inc)

# capping `annual_inc`
# quantile(df$annual_inc, probs = seq(0.9, 1, 0.001))
# cap_value <- quantile(df$annual_inc, 0.995)
...
# capping `dti`
# quantile(df$dti, probs = seq(0.9, 1, 0.001))
cap_value <- quantile(df$dti, 0.999)
...
# capping `revol_bal`
# quantile(df$revol_bal, probs = seq(0.9, 1, 0.001))
# cap_value <- quantile(df$revol_bal, 0.996)
...
df$pub_rec_non_bankruptcies <- forcats::fct_collapse(
  ...
  "0" = "0",
  "1+"= as.character(1:max(df$pub_rec_non_bankruptcies)))
...
df$pub_rec_bankruptcies <- forcats::fct_collapse(
  ...
  "0" = "0",
  "1" = "1",
  "2+" = as.character(2:max(df$pub_rec_bankruptcies)))
...
```{r}
# create `non_open_acc` as difference between `total_acc` and `open_acc`
df$non_open_acc <- df$total_acc - df$open_acc

# log transforming `non_open_acc`
# df$non_open_acc_log <- log1p(df$non_open_acc)

# capping `non_open_acc`
# quantile(df$non_open_acc, probs = seq(0.9, 1, 0.001))
# cap_value <- quantile(df$non_open_acc, 0.998)
# df$non_open_acc_cap <- ifelse(df$non_open_acc > cap_value, cap_value, df$non_open_acc)

# combining `non_open_acc` into 10 bins
df$non_open_acc_bins <- cut(
  df$non_open_acc,
  breaks = quantile(df$non_open_acc, probs = seq(0, 1, 0.1)),
  include.lowest = TRUE
)

# remove `open_acc`, `total_acc`, and `non_open_acc`
df <- df %>% dplyr::select(-open_acc, total_acc, -non_open_acc)

table(df$non_open_acc_bins)
```

**We then chose to create `non_open_acc` as the difference between `pub_rec` and `total_acc` to replace `open_acc` and accommodate for double-counting open accounts and any multicollinearity between `total_acc` and `open_acc`. `total_acc` is zero-dominated and thus very right-skewed. `non_open_acc` is very right-skewed with a couple large outliers. We chose to bin `non_open_acc` into 10 relatively equal-width bins to reduce the total levels and retain interpretability. Depending on the performance of `open_acc_bins` in the modeling phase, we may choose to backtrack and either perform a log1p (accounting for values of "0") transformation on `open_acc` to make the distribution close to normal and more symmetric, or cap `open_acc` to the 99.8th percentile to limit the influence of the large outliers on future analysis.**
...
# capping `revol_util`
# quantile(df$revol_util, probs = seq(0.9, 1, 0.001))
cap_value <- quantile(df$revol_util, 0.999)
...
# recode `mths_since_last_delinq` to years
df$yrs_since_last_delinq <- df$mths_since_last_delinq %>%
  ...
  fct_collapse(
    ...
    ">6yr" = as.character(72:max(as.numeric(df$mths_since_last_delinq[df$mths_since_last_delinq != "Never"]))))
    # "7-8yr"   = as.character(84:95),
    # "8-9yr"   = as.character(96:107),
    # "9-10yr"  = as.character(108:119),
    # "10-11yr" = as.character(120:131),
    # "11-12yr" = as.character(132:143),
    # "12-13yr" = as.character(144:155),
    # "13-14yr" = as.character(156:max(as.numeric(df$mths_since_last_delinq[df$mths_since_last_delinq != "Never"]))))
...
df$last_credit_pull_d_years_since <- year(Sys.Date()) - year(df$last_credit_pull_d_date)

# impute missing values of `last_credit_pull_d_years_since as "Never"
...
# group `emp_title` into broad categories based on the most common words
...
df$term <- ifelse(df$term == " 36 months", "3 Years", "5 Years")
...
df <- df %>%
  ...
  mutate(mort_acc = ifelse(is.na(mort_acc), median(mort_acc, na.rm = TRUE), mort_acc)) %>%
  ungroup()
...
df$mort_acc <- forcats::fct_collapse(
  ...
  "5+" = as.character(5:max(df$mort_acc)))
