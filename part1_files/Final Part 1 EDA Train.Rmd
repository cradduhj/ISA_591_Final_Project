...
df$application_type <- forcats::fct_other(df$application_type, keep = "INDIVIDUAL", other_level = "OTHER")
...
**We chose to collapse `application_type` into "INDIVIDUAL" and "OTHER" as the other levels are extremely sparse.**

```{r}
# collapse `purpose` into the top 3 levels in terms of frequency and "OTHER" ("OTHER" includes all less frequent levels)
df$purpose <- forcats::fct_collapse(df$purpose, OTHER = c("renewable_energy", "educational", "wedding", "house", "other", "car", "moving", "vacation", "small_business", "medical", "major_purchase"))
...
df$delinq_2yrs <- forcats::fct_collapse(
  ...
  "0" = "0",
  "1" = as.character(1:max(df$delinq_2yrs))) # range from 1 to the max value of the variable
...
**`acc_now_delinq` is extremely zero-dominated and right-skewed. To combat this, we chose to convert `acc_now_delinq` to factor and collapse the categories into 2 levels: "0", "1+". We did this to reduce the number of levels, mitigate data sparsity, and classify larger values of `acc_now_delinq` as equal.**
...
df$pub_rec_bankruptcies <- forcats::fct_collapse(
  ...
  "0" = "0",
  "1+" = as.character(1:max(df$pub_rec_bankruptcies))) # range from 1 to the max value of the variable
...
rf_grade <- randomForest::randomForest(
  loan_default ~ grade,
  data = df,
  ntree = 500,
  importance = TRUE
...
# View basic results
print(rf_grade)
randomForest::importance(rf_grade)
```

```{r}
set.seed(123)

rf_subgrade <- randomForest::randomForest(
  loan_default ~ sub_grade,
  data = df,
  ntree = 500,
  importance = TRUE
)
...
# View basic results
print(rf_subgrade)
randomForest::importance(rf_subgrade)
```
...
rf_territory <- randomForest::randomForest(
  loan_default ~ territory,
  data = df,
  ntree = 500,
  importance = TRUE
)
...
# View basic results
print(rf_territory)
randomForest::importance(rf_territory)
```

```{r}
set.seed(123)

rf_region <- randomForest::randomForest(
  loan_default ~ region,
  data = df,
  ntree = 500,
  importance = TRUE
...
# View basic results
print(rf_region)
randomForest::importance(rf_region)
```
...
```{r}
saveRDS(df, file = "final_cleaned_train.rds")
```
